...
def ansible_task(
        variables, playbook, override_files=None, runif='passed',
    ...
    Arguments:
        variables (dict): Variable overrides to pass to ansible on the commandline.
        playbook (str): The path (from the working_dir) to the playbook to execute.
        override_files (list): A list of paths to pass as variable override files (optional).
    ...
    command = prefix + [
        'ansible-playbook',
        '-vvvv',
    ] + inventory + extra_options

    for name, value in sorted(variables.items()):
        command.append('-e {}={}'.format(name, value))

    if override_files:
        for override_file in sorted(override_files):
...
def generate_launch_instance(job, optional_override_files=[], runif="passed"):
    ...
    return job.add_task(ansible_task(
        ...
        override_files=optional_override_files,
        runif=runif,
    ))
...
def generate_run_migrations(job, sub_application_name=None, runif="passed"):
    ...
    if sub_application_name is not None:
        variables['SUB_APPLICATION_NAME'] = sub_application_name
